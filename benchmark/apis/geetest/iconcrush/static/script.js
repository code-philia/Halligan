function init(id) {
    document.addEventListener("DOMContentLoaded", async () => {
        const challenge_frame = document.querySelector("#geetest_iconcrush");
    ...
    constructor(id, challenge, grid, images) {
        ...
        for (let i = 0; i < 3; i++) {
            for (let j = 0; j < 3; j++) {
                ...
                const gridItem = challenge.querySelector(`#item_${i * 3 + j + 1}`);
                const gridImage = gridItem.querySelector("div");
                gridImage.style["backgroundImage"] = `url(data:image/png;base64,${image})`;
                gridItem.onclick = () => {
                    if (this.from == id) {
                        gridItem.style["backgroundColor"] = "#e7ecf5";
                        this.from = null;
                    } else if (this.from == null) {
                        gridItem.style["backgroundColor"] = "#C7D8F7";
                        this.from = [i, j];
                    } else if (this.from[0] >= 0 && this.from[1] >= 0) {
                        this.to = [i, j]
                        this.submit();
                    }
                }
