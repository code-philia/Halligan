...
def submit_challenge():
    ...
    solved = (
        # Rows
        any(len(set(row)) == 1 and row[-1] != 0 for row in state)
        or
        # Columns
        any(len(set(col)) == 1 and col[-1] != 0 for col in zip(*state))
        or
        # Diagonals
        len(set(state[i][i] for i in range(len(state)))) == 1
        and state[0][0] != 0
        or len(set(state[i][len(state) - i - 1] for i in range(len(state)))) == 1
        and state[0][len(state) - 1] != 0
    )
